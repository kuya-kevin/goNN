...
//input = 255*255 float64, feed forward to make hidden, feed forward to make final
func New(input, hidden, output int) (retVal *NN) {
	r := make([]float64, hidden*input)
	r2 := make([]float64, hidden*output)
	fillRandom(r, float64(len(r))) //fills a []float64 w/ random values, here fill w/ random values from normal dis
...
//tensor API:
//two structural types that fulfill tensor.Tensor interface: *tensor.Dense + *tensor.CS
...
func (nn *NN) Predict(a tensor.Tensor) (int, error) {
	...
	//result of the final activation is a slice of 10 float64?? check here, should be a slice of only 2 float64
...
func (nn *NN) Train(x, y tensor.Tensor, learnRate float64) (cost float64, err error) {
	...
	hidden := m.do(func() (tensor.Tensor, error) { return tensor.MatMul(nn.hidden, x) })             //matrix multiplication, calculates first hidden layer
	act0 := m.do(func() (tensor.Tensor, error) { return hidden.Apply(sigmoid, tensor.UseUnsafe()) }) //calcs first layer
	...
	// backpropagation.
	//derivation of the cost function with regard to the weights matrix
	...
	//end of derivative calculation of the cost with regard of the input matrix

	// gradient update: add the gradient to the values itself, multiply by the learn rate
...
//maybe struct: bring error to the main function, log.Fatal will be called
type maybe struct {
	err error
}
